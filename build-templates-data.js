...
const cand = {};
function add(ref, css, html, source) {
  if (!css || !css.trim()) return;
  // keep the ref exactly as data.js uses it (e.g. "RD0016b" — uppercase base, lowercase variant suffix)
  (cand[ref] = cand[ref] || []).push({ css, html: html || null, complete: isComplete(html), source });
}
...
if (fs.existsSync(OUT)) {
  for (const name of fs.readdirSync(OUT)) {
    ...
    add(ref, read(path.join(dir, "style.css")), read(path.join(dir, "index.html")), "export");
  }
}
...
Object.keys(cand).forEach((ref) => {
  ...
  const best = usable[0];
  assets[ref] = { css: best.css, html: best.complete ? cleanBody(best.html) : null };
});
...
const banner =
  ...
  "   PAGE_STYLES = reference/page-styles.css ; OVERLAY_ASSETS[ref] = {css, html|null}. */\n";
fs.writeFileSync(
